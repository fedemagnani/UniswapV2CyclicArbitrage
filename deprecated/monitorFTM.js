...
async function shoot(_tokens,_amount,_routers,_contract,_gasLimit,_gasPrice){
    var swappa = _contract.methods.multipleSwapMultiUni(_tokens,_routers,false)
...
var excess = 1.2 //minimum gain that triggers bot is fee*excess
...
;(async()=>{
    ...
    for(var z=0;;z++){
        ...
        var _gasLimit = await contrattoLifeguard.methods.multipleSwapMultiUni(_tokens,finalRouterPaths[0],true).estimateGas({value:_mockValue,from:userAddress})
            ...
            for(i=0;i<orderedReserves.length;i++){
                if(doesExistArbitrageWithLog(orderedReserves[i],orderedDecimals[i],swapFee)&&shotPaths.indexOf(finalTokenPaths[i])==-1){
                    ...
                    try{
                        ...
                        shoot(finalTokenPaths[i], fullArb["Input"],finalRouterPaths[i],contrattoLifeguard, _gasLimit,_gasPrice)
                    }catch(e){
                        console.log(e)
                    }
                    discord.arbitrageAlert(w_endpoint,fullArb,dexes.join("_"))
                }
            }
